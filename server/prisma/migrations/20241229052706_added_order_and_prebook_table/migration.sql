...
CREATE TABLE `orders` (
    ...
    `paymentMode` ENUM('CASH_ON_DELIVERY', 'ONLINE') NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
-- CreateTable
CREATE TABLE `prebooking_counts` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
...
-- AddForeignKey
ALTER TABLE `Prebooking` ADD CONSTRAINT `Prebooking_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
